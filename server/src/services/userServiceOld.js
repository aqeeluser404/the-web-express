...
module.exports.DeleteUserService = async (id) => {
    try {
        ...
        const deleteDocumentPromises = user.documents.map(doc => deleteDocumentFromImageKit(doc.fileId));
        await Promise.all(deleteDocumentPromises);

        // Remove all rentals from the user's rentals array
        const removeRentalPromises = user.rentals.map(rentalId => 
        ...
        await Promise.all(removeRentalPromises);

        // Delete the user
        await User.findByIdAndDelete(id);

        return { message: 'User, their documents, and rentals deleted successfully' };
    } catch (error) {
        throw error;
    }
};
